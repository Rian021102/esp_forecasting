...
def build_autoencoder(X_train):
    with tf.device('/device:GPU:0'):
        ...
        L1 = LSTM(32, activation='relu', return_sequences=True,
                  kernel_regularizer=regularizers.l1(0.00))(inputs)
        ...
        autoencoder = tf.keras.models.Model(inputs=inputs, outputs=output)
...
def main():
    ...
    plot_data(train,feat_name='Volt')
    scaled_train, scaled_test, scaler=preprocess_train(train, test, feat_name='Volt')
    time_steps=3
    X_train, X_test=create_data_test(scaled_train, scaled_test, time_steps, feat_name='Volt')
