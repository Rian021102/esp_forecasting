...
def clean_train(train):
    ...
    #train['TM'] = imputer.fit_transform(train[['TM']])
    ...
    train = train[(train['TM'] > lower_bound) & (train['TM'] < upper_bound)]
    print(train.shape)
    print(train.head())
    return train
...
def train_autoencoder(autoencoder, X_train, X_test,epochs=100,batch_size=32):
    ...
    modelcheckpoint = tf.keras.callbacks.ModelCheckpoint(
        filepath='/Users/rianrachmanto/miniforge3/project/esp_forecast_LSTM/model/autoencoder_temp.h5',
        monitor='val_loss',
        save_best_only=True)
...
def create_anomaly_df(test, reconstruction_errors_inv, threshold_inv, predicted, time_steps):
    ...
    if predicted.shape[0] == len(test_score_df):
        test_score_df['Predicted_Temp'] = predicted[:, -1]  # assuming the last value in each prediction sequence
    else:
...
def main():
    ...
    df=load_data(path)
    df_feat=select_feat(df, 'MHNW-1', feat_name='TM')
    train, test=train_test(df_feat)
    train=clean_train(train)
    test=clean_test(test)
    plot_data(train,feat_name='TM')
    scaled_train, scaled_test, scaler=preprocess_train(train, test, feat_name='TM')
    time_steps=2
    X_train, X_test=create_data_test(scaled_train, scaled_test, time_steps, feat_name='TM')
    ...
    test_score_df['Predicted_Temp'] = corrected_predicted_values_inv
    print(test_score_df.head())

    # Plot actual, predicted, threshold and anomaly
    plt.figure(figsize=(12, 6))
    plt.plot(test_score_df['Date'], test_score_df['TM'], color='blue', label='Actual')
    plt.plot(test_score_df['Date'], test_score_df['Predicted_Temp'], color='red', label='Predicted')
    plt.scatter(test_score_df.loc[test_score_df['anomaly'], 'Date'], test_score_df.loc[test_score_df['anomaly'], 'TM'], color='red', label='Anomaly')
